<script lang="ts">
	...
	function mkEl<T extends SVGElement>(tag: string, attrs: Record<string, string | number>): T {
	...
	function buildBars() {
		barEls = BARS.map(d => {
			const r = mkEl<SVGRectElement>('rect', {
				...
				rx: 1, fill: 'currentColor', opacity: d.op,
			});
			svgEl.appendChild(r);
			return r;
		});
	...
	async function scanIn() {
		barEls.forEach(r => { r.setAttribute('width', '0'); r.setAttribute('opacity', '0'); });
	...
	async function doIdle() {
		if (state !== 'bars') { scheduleIdle(); return; }
		state = 'transitioning';
...
>
	<div class="mark" aria-hidden="true">
		<svg bind:this={svgEl} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"></svg>
	</div>
